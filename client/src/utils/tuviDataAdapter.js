...
export const adaptBackendToFrontend = (backendData, userInfo) => {
  ...
  if (!houses || !Array.isArray(houses)) {
    console.error("adaptBackendToFrontend: houses is missing or not an array", backendData);
    return null;
  }
  ...
  const thienBan = {
    ...
    gioSinh: `Giờ ${input?.birthHour || 0}`,
    gioiTinh: input?.gender === "nam" ? "Nam" : "Nữ",
    namXem: new Date().getFullYear(),
    banMenh: `${input?.can} ${input?.chi}`,
    cuc: input?.menh ? `${input.menh} Cục` : "",
