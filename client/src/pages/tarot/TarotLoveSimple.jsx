...
import LoveSimpleResult from '@components/tarot/loveSimple/LoveSimpleResult'
...
const TarotLoveSimple = () => {
  ...
  // Gọi API khi có đủ cả question, đã chọn 3 lá bài VÀ đã flip cả 3 lá
  useEffect(() => {
    const fetchReading = async () => {
      // Chỉ gọi API khi có đủ cả question, đã chọn 3 lá, đã flip 3 lá, chưa loading và chưa có result
      if (!question || selectedCards.length !== 3 || flippedCards.length !== 3 || loading || result) {
        return
      }
      ...
      try {
        ...
        const response = await tarotService.getLoveSimpleReading({
          question: question,
        ...
        console.error('Error fetching love simple reading:', err)
      } finally {
        setLoading(false)
      }
    }
