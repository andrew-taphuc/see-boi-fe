...
import LoveDeepResult from '@components/tarot/loveDeep/LoveDeepResult'
...
const TarotLoveDeep = () => {
  ...
  // Gọi API khi có đủ cả question, đã chọn 5 lá bài VÀ đã flip cả 5 lá
  useEffect(() => {
    const fetchReading = async () => {
      // Chỉ gọi API khi có đủ cả question, đã chọn 5 lá, đã flip 5 lá, chưa loading và chưa có result
      if (!question || selectedCards.length !== 5 || flippedCards.length !== 5 || loading || result) {
        return
      }
      ...
      try {
        ...
        const response = await tarotService.getLoveDeepReading({
          question: question,
          card1: flippedCards[0],
          card2: flippedCards[1],
          card3: flippedCards[2],
          card4: flippedCards[3],
          card5: flippedCards[4]
        })
        ...
        console.error('Error fetching love deep reading:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  }, [question, selectedCards.length, flippedCards.length])
  ...
  const handleCardSelected = (cardName) => {
    ...
    setSelectedCards(prev => {
      ...
      // Nếu chưa đủ 5 lá, thêm vào
      if (prev.length < 5) {
        return [...prev, cardName]
      }
      // Nếu đã đủ 5 lá, không thêm nữa
      return prev
    })
  }
  ...
  const getCardsData = () => {
    ...
    return [
      ...
      tarotCardsData.find(card => card.card_name === result.cards.card4.name),
      tarotCardsData.find(card => card.card_name === result.cards.card5.name)
    ].filter(Boolean)
  }
